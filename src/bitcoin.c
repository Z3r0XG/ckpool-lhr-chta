...
/* Take a CHTA address and do some sanity checks on it, then send it to
 * cheetahcoind to see if it's a valid address */
bool validate_address(connsock_t *cs, const char *address, bool *script, bool *segwit)
{
	...
	if (!json_is_true(valid_val)) {
		LOGDEBUG("CHTA address %s is NOT valid", address);
		goto out;
	}
	ret = true;
	tmp_val = json_object_get(res_val, "isscript");
	if (unlikely(!tmp_val)) {
		/* All recent cheetahcoinds with wallet support built in should
		 * support this, if not, look for addresses the braindead way
		 * to tell if it's a script address. */
		LOGDEBUG("No isscript support from cheetahcoind");
		if (address[0] == '3')
			*script = true;
		/* CHTA has no segwit/bech32; if address doesn't start with
		 * 'C' (P2PKH mainnet), '3' (P2SH mainnet), or 'c' (P2PKH
		 * testnet), reject it. */
	...
	*segwit = json_is_true(tmp_val);
	LOGDEBUG("CHTA address %s IS valid%s%s", address, *script ? " script" : "",
		 *segwit ? " segwit" : "");
out:
	if (val)
		json_decref(val);
...
/* CHTA supports csv but not segwit; request csv-only templates */
static const char *gbt_req = "{\"method\": \"getblocktemplate\", \"params\": [{\"capabilities\": [\"coinbasetxn\", \"workid\", \"coinbase/append\"], \"rules\" : [\"csv\"]}]}\n";

/* Request getblocktemplate from cheetahcoind already connected with a connsock_t
...
/* Request getblockcount from cheetahcoind, returning the count or -1 if the call
 * fails. */
...
/* Request getblockhash from cheetahcoind for height, writing the value into *hash
...
/* Request getbestblockhash from cheetahcoind. cheetahcoind 0.9+ only */
